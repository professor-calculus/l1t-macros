...
class TL1et_1d_plot : public TL1Plots
{
public:
    ~TL1et_1d_plot();
...
void TL1et_1d_plot::InitPlots()
{
    fRootFile = TFile::Open(Form("%s/et_1d_plot_%s.root", this->GetOutDir().c_str(), this->GetOutName().c_str()), "RECREATE");
    fPlot.emplace_back(new TH1F(Form("et_1d_plot_%s",fXName.c_str()),"", fXBins.size()-1,&(fXBins)[0]));
    //fPlot.back()->Sumw2();
    fPlot.back()->SetDirectory(0);
    fPlot.back()->GetXaxis()->SetTitle(fXTitle.c_str());
    for(int ipu=0; ipu<this->GetPuType().size(); ++ipu)
    {
        fPlot.emplace_back(new TH1F(Form("et_1d_plot_%s_%s",fXName.c_str(),this->GetPuType()[ipu].c_str()),"", fXBins.size()-1,&(fXBins)[0]));
        //fPlot.back()->Sumw2();
        fPlot.back()->SetDirectory(0);
        fPlot.back()->GetXaxis()->SetTitle(fXTitle.c_str());
    }
...
void TL1et_1d_plot::Fill(const double & xVal, const double & yVal, const double & nVtxVal)
{
    //fPlot[0]->Fill(xVal,yVal);
    
    for(int ipu=0; ipu<this->GetPuType().size(); ++ipu)
    {
        if( yVal >= this->GetPuBins()[ipu] && nVtxVal < this->GetPuBins()[ipu+1] )
            fPlot[ipu]->Fill(xVal);
...
TH1F * TL1et_1d_plot::GetCumulative(TH1F * plot)
{
    std::string newName = Form("cumulative_%s",plot->GetName());
    TH2F * temp = (TH1F*)plot->Clone(newName.c_str());
    temp->SetDirectory(0);
...
void TL1et_1d_plot::PlotE2(TH1F * plot, bool puOn)
{
...
void TL1et_1d_plot::DrawCmsStamp()
{
...
void TL1et_1d_plot::SetX(const std::string & xName, const std::string & xTitle)
{
    fXName = xName;
    fXTitle = xTitle;
}

void TL1et_1d_plot::SetXBins(const std::vector<double> & xBins)
{
    fXBins = xBins;
}
